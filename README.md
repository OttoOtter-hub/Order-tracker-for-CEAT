...
## Фаза 18: локализация ошибок API через коды (не задеплоено, коммит `eb645e7`)
...
- **EN-проход по UI** (обе роли, все страницы и диалоги, локально): в коде
  ...
  EN-интерфейсе не найдено. Остаются по-русски (вне UI): заголовки в трёх
  Excel-выгрузках (PI, бэкордер, готово к отгрузке) и двуязычные письма Фазы 13 —
  не трогались.
