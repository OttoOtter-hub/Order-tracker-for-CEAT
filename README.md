...
|---|---|---|
| `GET /ready-to-ship` | client (свой `customer_id`); ops с `?customerId=` | нераспределённые строки + контейнеры с аллокациями, `fillPercent`/`isOverfilled`, счётчик маркировки "N из M", `totalPossibleContainers`, `canConfirm`. При первом обращении создаёт пустые слоты |
...
| `POST /ready-to-ship/undo-last` | client | откат самого нового действия среди неподтверждённых контейнеров; `404`, если откатывать нечего |
...
| `POST /containers/:id/unlock` | ops | `is_confirmed=false` только у этого контейнера; файлы маркировки не трогает |
| `POST /container-allocations/:id/marking-file` | client | один файл на строку — повторная загрузка заменяет; только для подтверждённого контейнера |
...
- После: пустые слоты удалены (а не оставлены — ленивая инициализация не
  "подтягивает" их число при следующей загрузке бэкордера, устаревшее число
  слотов клиент увидел бы при первом заходе), таблицы Фазы 8 пусты, отпечатки
...
Открытый вопрос дизайна: число слотов создаётся один раз при первом заходе
клиента (как в ТЗ). Если следующая загрузка бэкордера потребует больше
контейнеров, слоты сами не добавятся — поможет `undo-all` (пересоздаёт по
актуальному плану), но фронтенду стоит это учесть.
...
| Ready to ship | `GET /ready-to-ship` (обе роли), `POST /ready-to-ship/move`, `/undo-last`, `/undo-all`, `/confirm` (client-only), `POST /containers/:id/unlock` (ops-only), `POST`/`DELETE /container-allocations/:id/marking-file` (client-only), `GET /container-allocations/:id/marking-file/download` (обе роли) — см. "Готово к отгрузке" выше |
