...
## Уровень доступа «администратор» — `is_admin` (задеплоено на прод 2026-09-25, коммит `133417b`)

**Деплой.** Порядок: **сначала миграция, потом код.** Сборки загружены по
SSH-ключу, SHA-256 сверен, `dist.new` / `ceat-frontend.new` рядом;
`migration:show` — ровно одна новая `AddUserIsAdmin1790900000000`, прогнана
при работающем прежнем коде (15/15). Read-only сверка: колонка `is_admin
boolean NOT NULL DEFAULT false`, ограничение `chk_users_admin_is_ops` на месте;
администратор — только самый ранний ops (`ops@ceat.com`), второй ops и client
— `false`. `diff -rq dist dist.new` — ровно файлы фазы. `stop` →
`mv dist dist.bak-20260925-admin` → `mv dist.new dist` → `start`: `GET /auth/me`
и `PATCH /users/:id/set-admin` зарегистрированы, 55 маршрутов, ошибок нет.
Фронтенд `index-CHF9T9Rd.js`, прежний — `ceat-frontend.prev-before-admin`,
`nginx -t` → `reload`. Публичные проверки (домен по https и IP): страница,
бандл, прямые ссылки — `200`; `/api/users`, `/api/audit-log`, `/api/auth/me`,
`set-admin` без токена — `401 UNAUTHORIZED`; неверный логин —
`INVALID_CREDENTIALS`; http домена — `301`; порт 3000 закрыт; ufw — 22/80/443.
Вход под реальными учётными записями (меню/редирект у обычного ops, оба
раздела у администратора) проверяет владелец.

Третий уровень: ops-пользователь может быть ещё и администратором. Только
администратор управляет пользователями и видит журнал действий; обычный ops
работает со всем остальным как прежде.

- **Колонка `users.is_admin`** (миграция `AddUserIsAdmin1790900000000`):
  `boolean NOT NULL DEFAULT false` и check-ограничение
  `chk_users_admin_is_ops` — администратором может быть только `role = 'ops'`.
  Миграция делает администратором ровно одного существующего пользователя:
  самого раннего по `created_at` **активного** ops (если активных ops нет —
  самого раннего ops вообще); все остальные — `false`.
- **Доступ.** Декоратор `@AdminOnly()` (вместе с `@Roles(Role.OPS)`) на
  `UsersController` и `AuditLogController`: `GET/POST /users`,
  `PATCH /users/:id/deactivate|reactivate|set-admin`, `GET /audit-log`. Client
  по-прежнему получает `403 OPS_ONLY_ACTION`, ops без прав администратора —
  `403 ADMIN_ONLY_ACTION`. `isAdmin` для проверки берётся из базы
  (`JwtStrategy`, через тот же кэш, что и `is_active`, со сквозной записью), а
  не из токена — выдача и снятие прав действуют сразу, без повторного входа.
- **`POST /users`** принимает необязательный `isAdmin` (по умолчанию `false`);
  для client — `400 ADMIN_REQUIRES_OPS`.
- **`PATCH /users/:id/set-admin`** `{ "isAdmin": true|false }` — выдать/снять
  права у ops-пользователя (client — `400 ADMIN_REQUIRES_OPS`, неизвестный —
  `404`). Последний **активный** администратор прав не лишается — ни чужими
  руками, ни своими (`400 LAST_ADMIN`); деактивированный администратор в счёт
  не идёт. Повторная установка того же значения — без изменений и без записи в
  журнал. Журнал: `user.admin_granted` / `user.admin_revoked`; у
  `user.created` в деталях — `isAdmin`.
- **`GET /auth/me`** — текущий пользователь с `isAdmin` из базы. `isAdmin` есть
  и в JWT, но только для меню; фронтенд при старте (и после изменения своих
  прав) перечитывает `/auth/me`, так что меню следует за базой и у сессий,
  открытых до изменения.
- **Фронтенд.** Пункты «Пользователи» и «Журнал действий» видит только
  администратор; обычный ops их не видит, а прямой адрес уводит на список PI.
  В «Пользователях» — колонка «Администратор»: переключатель на каждой
  ops-строке (у client — «—»); у последнего активного администратора он
  неактивен с пояснением. В форме создания для роли ops — флажок
  «Администратор».
- **Тесты:** backend 513 (+27): `users.admin.spec.ts` (создание с `isAdmin`,
  выдача/снятие, последний администратор, деактивированный не в счёт,
  client/404, кэш статуса, `JwtStrategy` берёт права из базы, SQL миграции),
  `users.guard.spec.ts` (все шесть маршрутов: client — `OPS_ONLY_ACTION`,
  обычный ops — `ADMIN_ONLY_ACTION`, администратор — доступ; `/auth/me` и
  смена пароля открыты всем), обновлён guard-тест журнала.
- **Проверено живьём** (локально): миграция на базе с тремя ops разных дат
  сделала администратором только самый ранний; ограничение не даёт сделать
  администратором client. API: обычный ops — `403 ADMIN_ONLY_ACTION` на всех
  шести маршрутах (при этом «Готово к отгрузке» и PI — `200`), client — `403
  OPS_ONLY_ACTION`, администратор — `200`; выдача прав действует на том же
  токене; `LAST_ADMIN`; записи в журнале. В браузере: меню обычного ops без
  двух пунктов и редирект с прямых адресов; у администратора — колонка,
  переключатели, блокировка последнего, флажок в форме; старая сессия без
  `isAdmin` подхватывает его из `/auth/me`. Тестовые пользователи
  деактивированы.
- **При деплое:** сначала миграция, затем backend и фронтенд вместе; после —
  назначить администратором учётную запись владельца (по его подтверждению).

## Визуальный стиль CEAT (задеплоено на прод 2026-09-25, коммит `06fad1f`)

**Деплой.** Только фронтенд, backend и база не тронуты. Сборка с
`VITE_API_URL=/api` (`index-DhEOdts6.js`, `index-5ElwpgR0.css`, 5 файлов
Manrope), загружена по SSH-ключу, SHA-256 сверен; распакована рядом как
`ceat-frontend.new`, прежняя — `ceat-frontend.prev-before-rebrand`; `nginx -t` →
`reload`. Публичные проверки по домену (https) и по IP (http): страница, бандл,
CSS и шрифт (`font/woff2`) — `200`, в CSS новые токены, прямые ссылки SPA —
`200`, неверный логин — `401 INVALID_CREDENTIALS`; http домена — `301` на https;
порт 3000 снаружи закрыт; ufw — 22/80/443.

Только визуальный слой — цвета, шрифт, скругления; логотипов нет, вёрстка и
компоненты не менялись. Всё в токенах `frontend/src/index.css` (Tailwind v4,
переменные shadcn).

- **Палитра снята с ceat.com** (их CSS-переменные: `--accent-color: #f5822d`,
  тёмно-синий `#23283c`, синий `#0054a8`, почти чёрный текст, белые / `#f8f8f8`
  поверхности). Светлая тема — почти белый фон, белые карточки; тёмная — почти
  чёрный фон и приглушённо-белый (не чисто белый) текст.
- **Оранжевый CEAT** — кнопки, активный пункт меню (оранжевый тон и полоса
  слева), фокус, выбранный контейнер в модалке. **Тёмно-синий** — заголовки
  разделов/карточек/диалогов и нейтральные бейджи (`secondary`).
- **Контраст считан, не на глаз** (WCAG): белый текст на оранжевом CEAT — всего
  2,6:1, поэтому на оранжевых кнопках почти чёрный текст (7,3:1), а оранжевый
  *текст* (ссылки) в светлой теме — более тёмный `--link` (5,6:1). Все пары
  текст/фон тем — не ниже 4,5:1 (бейджи состояний 6–8:1).
- **Состояния сохранили смысл, но разведены с оранжевым:** красный (переполнение,
  ошибки) сдвинут к малиновому; «почти заполнен» и «внимание» (частично
  разблокирован, нет маркировки, «Ожидается прибытие», переопределённые даты) —
  горчично-золотой; строки приоритета — бледно-лимонные; «готово» (файлы в
  «Отгружено», «Прибыл», подписан) — зелёный; нормальная шкала заполнения —
  синий CEAT (оранжевый оставлен только для действий). Жёстко прописанные
  классы `amber/yellow/green/blue/purple/gray-*` заменены на семантические
  токены `success`, `warning`, `info`, `archived`, `idle`, `highlight`,
  `success-row` (у каждого свой вариант для тёмной темы).
- **Шрифт** — Manrope (геометрический, как их Poppins, но с кириллицей — у
  Poppins её нет), self-hosted через `@fontsource-variable/manrope` вместо
  Geist; табличные цифры (`tabular-nums`) поддерживаются. Скругление — 8 px, как
  у карточек на ceat.com.
- **Проверено живьём** в обеих темах (ops и client): логин, список PI, карточка
  PI со строками приоритета, «Готово к отгрузке» (>100% / 90–100% / норма, OK to
  mix), модалка «Переместить», «Отгружено» с зелёными строками и маркерами
  прибытия, тосты. Тестовые данные локально возвращены как были.
...
- **`POST /ready-to-ship/unlock-all?customerId=…`** — только ops
  ...
  без `customerId` — `400 CUSTOMER_ID_REQUIRED`). Одной транзакцией
  разблокирует все позиции всех контейнеров клиента, где есть хотя бы одна
  заблокированная (OK to mix — наравне с номерными; частично разблокированный
  — только оставшиеся заблокированные позиции). Логика общая с поштучной
  разблокировкой контейнера: `unlock()` и `unlockAll()` вызывают один
  `unlockContainerPositions` (снять `is_locked`, очистить `confirmed_at/by`).
- **Ответ** `{ customerId, containersUnlocked, positionsUnlocked, containers:
  [{ id, label, positionsUnlocked }] }`. Разблокировать нечего — не ошибка:
  нули и пустой список, журнал и уведомления не пишутся.
- **Журнал** — одна запись `rts.unlocked_all` (сущность — клиент) в той же
  транзакции: `containersUnlocked`, `positionsUnlocked`, `containerLabels`.
- **Уведомления** — после коммита то же событие, что и у поштучной
  разблокировки (`container.reopened-for-client`), по одному на каждый
  разблокированный контейнер.
- **Файлы маркировки** не трогаются при разблокировке — как и раньше, файл
  позиции удаляется, только когда клиент меняет её количество.
- **Фронтенд.** Кнопка "Разблокировать все" у ops в шапке блока "Контейнеры"
  (рядом со счётчиками, над карточками с поштучной разблокировкой); неактивна,
  когда заблокированного нет. Диалог подтверждения с предупреждением (RU/EN),
  после — тост "Разблокировано — контейнеров: N, позиций: M" (или "нечего
  разблокировать"), данные перечитываются. В журнале действий — "Разблокированы
  все подтверждённые контейнеры" с числами и списком контейнеров.
- **Тесты:** backend 486 (+9, `ready-to-ship.unlock-all.spec.ts`): снимает lock
  со всех позиций всех подтверждённых контейнеров клиента, OK to mix наравне с
  номерными, чужой клиент не затронут, частично разблокированный — только
  оставшиеся позиции, журнал с верными числами в транзакции, уведомление на
  каждый контейнер, "нечего разблокировать" без ошибки и без записи, 403
  client, 400 без `customerId`, файлы маркировки уходят только при изменении
  позиции.
- **Проверено живьём** (локально): client подтвердил Container 1, Container 2 и
  OK to mix (4 позиции); client → `403`, ops без клиента → `400`; в браузере у
  ops (RU/EN) — кнопка, текст диалога, тост "контейнеров: 3, позиций: 4",
  карточки снова черновики и развёрнуты, кнопка неактивна; запись в журнале с
  деталями. Данные локально возвращены как были.
- **При деплое:** без миграции — backend и фронтенд вместе.

## Фаза 22: пустой "Container" для неразмещённого, OK to mix первым в модалке, название контейнера (задеплоено на прод 2026-09-24, коммит `76b2a21`)

**Деплой.** Порядок: **сначала миграция, потом код.** `dist.new` рядом (SHA-256
архивов сверен), `migration:show` — ровно одна новая
`AddShippingContainerName1790800000000`, прогнана при работающем прежнем коде
(14/14; колонка аддитивная и nullable). Read-only сверка: `name character
varying(30)`, nullable; 48 контейнеров (включая OK to mix, созданный при
открытии страницы после Фазы 21), ни у одного нет имени. `diff -rq dist
dist.new` — ровно файлы фазы. `stop` → `mv dist dist.bak-20260924-p22` → `mv
dist.new dist` → `start`: `PATCH /containers/:id/name` зарегистрирован, 52
маршрута, ошибок нет. Фронтенд с `VITE_API_URL=/api` (`index-DOdzoxm6.js`),
прежний — `ceat-frontend.prev-before-p22`, `nginx -t` → `reload`. Публичные
проверки: `/`, бандл и CSS — `200`; `/ready-to-ship`, `export-xlsx` и `PATCH
/containers/:id/name` без токена — `401 UNAUTHORIZED`; неверный логин —
`INVALID_CREDENTIALS`; ufw — только 22/80. Переименование и выгрузка на живых
данных клиента на проде не выполнялись.

- **Экспорт (`GET /ready-to-ship/export-xlsx`) — подпись "OK to mix" только для
  реального контейнера OK to mix.** Позиции (строка на каждую
  `ContainerLineAllocation`) по-прежнему идут под меткой своего контейнера, в
  том числе настоящий OK to mix из Фазы 21. А нераспределённый остаток строки
  (реально нигде не лежит) теперь идёт с **пустым** столбцом "Container", а не
  "OK to mix". Порядок строк: номерные контейнеры, прочие метки, OK to mix,
  затем неразмещённое.
- **Модалка "Переместить": OK to mix первым**, выше "Контейнер 1". Только в
  модалке — в списке карточек на странице он остаётся последним (порядок
  сервера). Сортировка — чистая функция `moveTargetOrder`
  (`frontend/src/lib/containerDisplay.ts`).
- **Название контейнера.**
  - Колонка `shipping_containers.name` (`varchar(30)`, nullable; миграция
    `AddShippingContainerName1790800000000`, аддитивная).
  - `PATCH /containers/:id/name` с `{ "name": "Ростов" }` (или `null` / пустая
    строка — сбросить; пробелы по краям отрезаются) — рядом с уже
    существующим `POST /containers/:id/unlock`. Отвечает свежим
    представлением "Готово к отгрузке". Только client-владелец: ops — `403
    CLIENT_ONLY_ACTION`, чужой контейнер — `404 NOT_FOUND`, OK to mix — `400
    OK_TO_MIX_NAME_NOT_ALLOWED`, длиннее 30 — `400 CONTAINER_NAME_TOO_LONG`, не
    строка — `400 CONTAINER_NAME_NOT_STRING`. Это пометка, не часть плана:
    подтверждённый контейнер тоже можно переименовать. Журнал —
    `rts.container_renamed` (метка, было/стало), только при реальном
    изменении, в той же транзакции.
  - "Отменить всё" пересоздаёт пустые слоты — слот, вернувшийся под той же
    меткой, получает своё название обратно.
  - В представлении у каждого контейнера `name` (у OK to mix всегда `null`).
  - Фронтенд: везде, где контейнер показан или выбирается — "Контейнер 3:
    Ростов" (как "номер PI: название"): заголовок карточки (развёрнутой и
    свёрнутой), модалка "Переместить", свободные слоты, предупреждение о
    переполнении, диалоги. У client на развёрнутой карточке номерного
    контейнера — поле "Название" с "Сохранить" (Enter тоже сохраняет).
  - Экспорт: новый столбец **"Container Name"** сразу после "Container" (пусто,
    если имени нет; у OK to mix и неразмещённых всегда пусто). Заголовок не
    просто "Name": так уже называется столбец названия карточки PI.
- **Тесты:** backend 477 всего (+14): `ready-to-ship.container-name.spec.ts`
  (владелец, 403/404/400, обрезка и сброс, журнал, подтверждённый контейнер,
  имя переживает "Отменить всё"), `update-container-name.dto.spec.ts`,
  экспорт — "в реальном OK to mix" (подпись) против "нигде не размещено"
  (пусто) и новый столбец с именем (построитель и `exportXlsx` целиком).
  Фронтенд получил `npm test` — встроенный тест-раннер Node (`node --test`,
  без новых зависимостей; Node 24 сам понимает TypeScript), тесты
  `src/**/*.test.ts` исключены из `tsc -b` приложения:
  `containerDisplay.test.ts` — OK to mix первым в модалке и формат "метка:
  имя".
- **Проверено живьём** (локально): переименование через API и в браузере
  (EN/RU — заголовок карточки, модалка, OK to mix первым и без поля имени,
  карточки — OK to mix последним), все отказы (OK to mix, ops, другой клиент,
  длина, тип); выгрузка — "Container 3" + "Rostov", одна строка "OK to mix"
  (реально там лежащая), 231 неразмещённая строка с пустым "Container".
  Данные локально возвращены как были. Кириллическое имя локально не
  проверить (локальная база в WIN1252), на проде — UTF-8.
- **При деплое:** сначала миграция, затем backend и фронтенд вместе.
...
сертификат Let's Encrypt, настроен владельцем проекта). Прямой доступ по IP
(`http://<server-ip>`) оставлен как запасной путь, пока домен не
зарекомендует себя.

- **nginx.** `sites-available/ceat-frontend` — блок `listen 443 ssl` для домена
  (им управляет certbot) и его блок `listen 80 default_server`: домен → `301`
  на https (путь и query сохраняются), любой другой Host → `404`. Из-за этого
  после выпуска сертификата отвалился доступ по IP (тоже `404`). Отдельным
  файлом `sites-available/ceat-frontend-ip` (симлинк в `sites-enabled`)
  добавлен блок `listen 80; server_name <server-ip>;` — тот же `root` и тот же
  прокси `/api/` → `localhost:3000`. Файл certbot не тронут (копия —
  `ceat-frontend.bak-20260925-before-ip`). Отключить запасной путь по IP —
  убрать симлинк `sites-enabled/ceat-frontend-ip` и `nginx -t` → `reload`.
- **`CORS_ORIGIN`** в `/opt/ceat-backend/.env` —
  `https://ttst.twelvetwenty.online,http://<server-ip>` (список через запятую,
  см. `src/main.ts`); копия прежнего `.env` — `.env.bak-20260925-cors`, остальные
  строки не менялись. Фронтенд ходит в `/api` того же origin, так что в браузере
  CORS не участвует — это на случай прямых кросс-доменных запросов.
- **Фронтенд** собран с `VITE_API_URL=/api` (относительный путь), поэтому один и
  тот же бандл работает и по домену, и по IP — пересборка не нужна.
- **ufw:** открыты только 22, 80, 443 (443 — под HTTPS); 3000 снаружи закрыт
  (проверено: соединение по таймауту). Node слушает `*:3000`, снаружи его
  закрывает ufw.
- **Проверено публично:** `https://` — `200`, сертификат валиден (Let's
  Encrypt, TLS 1.3, до 2026-12-23); `http://` домена — `301` на `https://`; по
  IP — страница, прямые ссылки SPA и `/api` снова `200`/`401`; неизвестный Host
  на 80 — `404`; CORS-preflight отдаёт `Access-Control-Allow-Origin` для домена
  и для IP и ничего — для чужого origin; неверный логин через домен —
  `401 INVALID_CREDENTIALS`. Реальный вход под учётной записью проверяет
  владелец.
- **Доступ к серверу для деплоя:** кроме пароля root, теперь есть ключ
  ed25519 (`claude-code-ceat-deploy` в `/root/.ssh/authorized_keys`); закрытый
  ключ — только на рабочей машине. Отозвать — удалить эту строку.
