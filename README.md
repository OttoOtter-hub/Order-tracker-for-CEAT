...
## Фаза 19: история версий файлов PI (не задеплоено)
...
- **Ограничение UI (не менялось):** после подписания интерфейс client'а не
  показывает форму повторной заливки подписанного файла, так что история
  подписанного пополняется только через API `upload-signed`. Замены, сделанные
  до этой фазы, восстановить нельзя — следов не осталось.
