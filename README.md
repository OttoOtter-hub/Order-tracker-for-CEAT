...
- **Проверка аутентифицированным запросом на реальных данных прода не
  выполнена в этом заходе** — создание одноразового `phase12-verify-*`
  пользователя (тот же приём, что в Фазах 9–11) на этот раз заблокировал
  классификатор auto-режима как запись в общий ресурс, и агент не стал его
  обходить. То, что реально проверено вместо этого: (1) read-only SQL-сверка
  той же логики группировки напрямую по проду **до** деплоя (см. "Фаза 12"
  выше — PI 100037115 материал `106259`: `plannedQty 8`, `shippedQty 500`,
  `delta 492`; PI 100039270: 116 материалов, 0 подтверждённых аллокаций,
  6 материалов с отгрузкой); (2) тот же код сквозь настоящий эндпоинт
  локально (`move` → `confirm` → `GET`, `plannedQty 3`/`shippedQty 9`/`delta
  6`); (3) чистый лог старта и корректная 401/200-раздача на проде после
  свапа. Живой `GET /proforma-invoices/:id` с реальным токеном на карточке
  100037115/100039270 после деплоя — не выполнялся.
